...
/**
 * Estimate and format transit duration based on distance (assuming 55 km/h average speed).
 *
 * @param float  $distanceKm
 * @param string $lang
 * @return string
 */
function estimateDuration(float $distanceKm, string $lang = 'en'): string
{
    // Assume average speed of 55 km/h for heavy trucks
    $seconds = ($distanceKm / 55) * 3600;
    ...
    if ($lang === 'sw') {
        $parts = [];
        if ($hours > 0) $parts[] = $hours . " saa";
        if ($minutes > 0) $parts[] = $minutes . " dak";
        if ($secs > 0 || empty($parts)) $parts[] = $secs . " sek";
        return implode(", ", $parts);
    } else {
        $parts = [];
        if ($hours > 0) $parts[] = $hours . " hr" . ($hours > 1 ? "s" : "");
        if ($minutes > 0) $parts[] = $minutes . " min" . ($minutes > 1 ? "s" : "");
        if ($secs > 0 || empty($parts)) $parts[] = $secs . " sec" . ($secs > 1 ? "s" : "");
        return implode(", ", $parts);
    }
}
