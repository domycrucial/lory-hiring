...
class Payment extends BaseModel
{
    ...
    public function processSimulated(array $data): array
    {
        ...
        // Credit lorry owner wallet
        try {
            $bookingModel = new Booking();
            $booking = $bookingModel->getFullDetail((int)$data['booking_id']);
            if ($booking && !empty($booking['owner_id'])) {
                $userModel = new User();
                $userModel->creditWallet((int)$booking['owner_id'], $payout);
            }
        } catch (Exception $e) {
            error_log('[OLHS Payment] Failed to credit wallet for booking ID ' . $data['booking_id'] . ': ' . $e->getMessage());
        }
