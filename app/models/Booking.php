...
class Booking extends BaseModel
{
    ...
    public function accept(int $bookingId, int $ownerId): bool
    {
        $sql  = "UPDATE bookings b
                 INNER JOIN lorries l ON l.id = b.lorry_id AND l.owner_id = :owner_id
                 SET b.status = 'accepted', b.accepted_at = NOW()
     ...
     * Also increments the lorry's total_trips counter.
     *
     * @param int $bookingId
     * @param int $ownerId
     * @return bool
    ...
    public function complete(int $bookingId, int $ownerId): bool
    {
        try {
            ...
            // Increment total_trips on the lorry
            $booking = $this->findById($bookingId);
            if ($booking) {
                $this->db->prepare(
                    "UPDATE lorries SET total_trips = total_trips + 1 WHERE id = :id"
